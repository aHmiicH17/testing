...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const logoutButton = document.getElementById('logout');
    const navLinks = document.querySelectorAll('.nav-link');
    const startLipsyncButton = document.getElementById('setup-btn');
    ...
    // Set up drag-and-drop and click-to-upload functionality for each drop zone
    [audioDropZone, videoDropZone].forEach(dropZone => {
        dropZone.addEventListener('dragover', handleDragOver);
        dropZone.addEventListener('drop', (e) => {
            handleDrop(e, dropZone.id.includes('audio') ? 'Audio' : 'Video');
    ...
    //start lipsyncing fuction
    startLipsyncButton.addEventListener('click', (e) => {
        e.preventDefault();
        fetch('/start_lipsync', {
            method: 'POST'
        }).then(response => {
            if (response.ok) {
                ...
                alert('Error in processing video');
            }
        });
    });
